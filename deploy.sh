...
start_infrastructure() {
    ...
    # Brief grace for container creation; the health poll below does the real waiting.
    echo "⏳ Waiting for services to be healthy..."
    sleep 3

    # Check service health. grep '(healthy)' matches only the literal "(healthy)"
    # status — NOT "(unhealthy)" / "(health: starting)" (the old 'healthy' substring did).
    echo "🔍 Checking service health..."
    for service in postgres kafka redis redis-gigavolve minio; do
        echo "Checking $service..."
        timeout 90s bash -c "until docker compose -f docker-compose.kafka.yml ps $service | grep -q '(healthy)'; do sleep 2; echo 'Still waiting for $service...'; done"
...
start_applications() {
    ...
    timeout 180s bash -c "until docker compose -f docker-compose.kafka.yml -f docker-compose.master-api.yml ps master-api | grep -q '(healthy)'; do sleep 2; echo 'Still waiting for Master API...'; done"
    ...
    # Wait for all runners concurrently — they start in parallel, so a serial
    # per-runner loop just adds (N-1)×(poll latency) of dead wall-clock.
    runner_wait_pids=()
    for i in $(seq 1 "${RUNNER_POOL_SIZE}"); do
        svc="runner-api-${i}"
        echo "Waiting for ${svc}..."
        timeout 180s bash -c "until docker compose -f docker-compose.kafka.yml -f \"${RUNNER_POOL_COMPOSE_FILE}\" ps ${svc} | grep -q '(healthy)'; do sleep 2; echo 'Still waiting for ${svc}...'; done" &
        runner_wait_pids+=("$!")
    done
    runner_wait_failed=0
    for pid in "${runner_wait_pids[@]}"; do
        if ! wait "${pid}"; then runner_wait_failed=1; fi
    done
    if [ "${runner_wait_failed}" -ne 0 ]; then
        echo "❌ One or more Runner API instances failed to become healthy"
        exit 1
    fi
    echo "✅ Runner API pool is healthy"
    ...
    timeout 180s bash -c "until docker compose -f docker-compose.kafka.yml -f docker-compose.master-api.yml -f docker-compose.web-ui.yml ps web-ui | grep -q '(healthy)'; do sleep 2; echo 'Still waiting for Web UI...'; done"
