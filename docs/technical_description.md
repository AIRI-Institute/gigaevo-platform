...
The default local deployment uses a Compose-managed runner pool. `make dev`, `make deploy`, and bundle flows generate a runner-pool Compose file and start `runner-api-N` containers from one shared runner image. In this mode, Master API does not build or create runner containers itself; it discovers the already-started containers, monitors their health, and assigns experiments to them. Each Runner instance still operates independently with its own dedicated set of worker processes, preserving isolation and resource management.
...
runner:
  ...
  pool_size: 3                 # Number of local runner containers
  pool_host_prefix: "runner-api-"
  manage_containers: false     # Compose starts runner containers
  health_monitoring_enabled: true
```

In the repo-level container flows, `RUNNER_POOL_SIZE` feeds the generated Compose file and changes the number of local runner containers on the next stack apply.
...
**Health Monitoring:**
The system implements continuous health checks every 30 seconds to monitor the status of all Runner instances. In the default Compose-managed mode, unhealthy instances are surfaced quickly for restart or reconciliation by deployment tooling and operator actions. Real-time status synchronization is maintained between Docker containers and the database, providing accurate and up-to-date information about system health.
...
Every experiment is broken down into discrete tasks that can be executed independently and in parallel. The typical task sequence includes experiment workspace preparation, code generation and configuration, actual experiment execution, and results collection and analysis. This task-based approach allows different stages of experiments to progress simultaneously when dependencies permit.
...
### Horizontal Scaling
The system supports horizontal scaling across multiple dimensions. In the default local deployment, new Runner instances are added by increasing `RUNNER_POOL_SIZE` (or `runner.pool_size`) and reapplying the stack so the generated runner-pool Compose file is refreshed. The worker count per instance can be scaled independently, enabling fine-tuned resource allocation based on experiment requirements. Geographic distribution is also supported, allowing runners to be deployed across different servers or cloud regions for optimal performance and redundancy.
...
The local filesystem provides temporary storage for working data during experiment execution. Runner containers use a prebuilt `gigaevo-core` tree under `/opt/gigaevo-core`, including a prepared virtual environment. Temporary experiment files are created and managed under that root (`problems/*`, `outputs/*`, `config/llm/custom.yaml`), providing workspace for intermediate computations. Local computation results are cached to reduce network traffic and improve performance for frequently accessed data.

## Integration Capabilities

### GigaEvolve Framework Integration
The system provides seamless integration with the GigaEvolve framework through image-baked core preparation at build time. Experiments leverage the GigaEvolve execution entrypoint (`run.py`) directly, maintaining compatibility with existing workflows. The integration supports GigaEvolve's native configuration and parameter systems, allowing users to work with familiar interfaces and methodologies. Native handling of GigaEvolve output formats ensures that results can be processed and displayed without additional conversion steps.

### External System Integration
GigaEvo integrates with external enterprise systems through multiple connectivity options. GitHub integration enables secure build-time access to private repositories using Personal Access Tokens, supporting collaborative development workflows. MinIO S3-compatible storage facilitates enterprise cloud storage integration, enabling seamless data management within existing infrastructure. PostgreSQL with connection pooling supports enterprise database requirements, while health check endpoints enable integration with enterprise monitoring and alerting systems.
...
```bash
# Deploy with 4 runner instances
RUNNER_POOL_SIZE=4 make deploy
