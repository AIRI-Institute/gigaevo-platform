...
from .experiment_sse import experiment_event_stream
...
async def _experiment_event_stream(
    ...
    *,
    ...
    """Server-side poll → SSE frames for a chain experiment.

    Moves CARE's client-side ``_poll_experiment_events`` loop onto the
    Platform: poll status + live runner metrics and push status / fitness /
    program frames the EvolutionScreen already understands. The loop itself
    (frame rules + heartbeat + duration/idle safety caps) lives in the
    dependency-free :func:`experiment_event_stream` so it can be unit-tested
    without the app; this wrapper just binds it to the service."""

    async def _status():
        return await service.get_experiment_status(experiment_id)

    async def _results():
        return await service.get_experiment_results(experiment_id)

    async for frame in experiment_event_stream(
        _status,
        _results,
        is_disconnected,
        experiment_id,
...
async def stream_experiment_events(
    ...
    """Live SSE stream of a chain experiment's progress (CARE §P4.2).

    404s upfront for an unknown experiment so we don't open a dead stream.
    """
    initial = await service.get_experiment_status(experiment_id)
    if not initial:
        raise HTTPException(status_code=404, detail="Experiment not found")
    return StreamingResponse(
        _experiment_event_stream(service, experiment_id, request.is_disconnected),
        ...
        try:
            if _service_manager and hasattr(_service_manager, "creation_service"):
                ...
                chain_spec: Dict[str, Any] = {
                    ...
                    "dataset_size": chain_experiment.dataset_size,
                    "test_size": chain_experiment.test_size,
                    "python_code": getattr(chain_experiment, "python_code", None),
