...
class RunnerConfig(BaseModel):
    ...
    # How many experiments a SINGLE runner may execute concurrently.
    # Default 1 preserves the historical binary-capacity behaviour
    # (one experiment per runner). Set >1 to let one runner host several
    # experiments at once — the runner is per-experiment-id internally, so
    # this lets CARE run multiple evolutions independently without
    # provisioning more runner containers. (Env: RUNNER__MAX_CONCURRENT_EXPERIMENTS_PER_RUNNER)
    max_concurrent_experiments_per_runner: int = 1
