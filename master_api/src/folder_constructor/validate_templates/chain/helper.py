...
# Configuration from template placeholders (substituted by chain_experiment_builder)
VALIDATION_TYPE = "${validation_type}"
METRIC = "${metric}"

_CONTEXT_SUFFIX_QA = "\n\nSolve the problem and compute the correct answer."
_SUMMARIZATION_MAX_WORDS = 40


def _is_rouge_metric() -> bool:
    return METRIC in ("ROUGE-1", "ROUGE-2", "ROUGE-L")


def _is_summarization_task() -> bool:
    return _is_rouge_metric()
...
def _finish_context(context_text: str) -> str:
    if _is_summarization_task():
        return context_text
    return f"{context_text}{_CONTEXT_SUFFIX_QA}"
...
def _format_row_as_context(row: pd.Series, target_column: str) -> str:
    ...
    context_text = "\n\n".join(context_parts)
    return _finish_context(context_text)
...
Write exactly one concise sentence that captures the main idea of the input text.
Use the same language as the input. Do not add markdown, quotes, labels, or preamble."""
...
def _strip_markdown(text: str) -> str:
    """Remove common markdown wrappers from LLM output before extraction."""
    if not text:
        return ""
...
def _extract_summarization_prediction(text: str) -> str:
    """ROUGE/summarization: last line when multi-line, else whole short text — never numeric."""
    lines = [line.strip() for line in text.split("\n") if line.strip()]
    if len(lines) > 1:
        return lines[-1]
    if len(text.split()) <= _SUMMARIZATION_MAX_WORDS:
        return text.strip()
    return lines[-1] if lines else text.strip()


def _find_raw_step(raw_steps: list, step_number: int) -> dict | None:
    for step in raw_steps:
        if isinstance(step, dict) and step.get("number") == step_number:
            return step
    return None


def _is_marked_output_step(raw_step: dict | None) -> bool:
    if not raw_step:
        return False
    return bool(raw_step.get("is_output_step") or raw_step.get("output_step"))


def _select_scoring_output(
    chain_spec: ChainSpec,
    step_outputs: list[str],
    chain_config: Dict[str, Any] | None = None,
) -> str:
    """Pick step output used for fitness scoring (not always the last step)."""
    if not step_outputs:
        return ""

    raw_steps = (chain_config or {}).get("steps") or []

    for idx, step in enumerate(chain_spec.steps):
        raw = _find_raw_step(raw_steps, step.number)
        if _is_marked_output_step(raw) and idx < len(step_outputs):
            return step_outputs[idx]

    if _is_summarization_task():
        llm_indices = [
            i for i, s in enumerate(chain_spec.steps) if isinstance(s, LLMStep)
        ]
        if len(llm_indices) >= 2:
            penultimate = llm_indices[-2]
            if penultimate < len(step_outputs):
                return step_outputs[penultimate]
...
def _extract_prediction(text: str) -> str:
    ...
    For ROUGE/summarization: last line or entire short text; no numeric fallback.
    For QA/math:
    ...
    if _is_summarization_task():
        return _extract_summarization_prediction(text)
...
def _run_split(
    ...
    for i, cr in enumerate(chain_results):
        sample = dataset[i]
        output_text = _select_scoring_output(
            chain_spec, cr.step_outputs, chain_config
        )
