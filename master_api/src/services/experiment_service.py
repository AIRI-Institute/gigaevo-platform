...
class ExperimentService:
    ...
    async def _fetch_programs(
        self, client: Any, problem_uuid: str, *, limit: int = 500
    ) -> List[Dict[str, Any]]:
    ...
    async def _read_live_evolution_metrics(
        ...
        programs = await self._fetch_programs(client, problem_uuid)
        if programs:
            out["generation"] = current_display_generation(programs)
            history = fitness_history_from_programs(programs)
            if history:
                out["fitness_history"] = history
            if "best_fitness" not in out:
                best = 0.0
                found = False
                for program in programs:
                    ...
                    if isinstance(fit, (int, float)) and fit > -999:
                        best = max(best, float(fit))
                        found = True
                if found:
        ...
        try:
            frontier = await self._read_frontier_programs(
                client, problem_uuid, programs=programs
            )
        except Exception as exc:
            logger.debug(
