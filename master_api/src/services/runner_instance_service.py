...
class RunnerInstanceService:
    ...
    async def _reconcile_busy_runners_loop(self) -> None:
        ...
        concurrency_limit = max(
            1, int(getattr(self.config.runner, "max_concurrent_experiments_per_runner", 1) or 1)
        )

        logger.info(
            f"Starting runner pool reconciler loop (interval={interval}s, "
            f"per_runner_concurrency={concurrency_limit})"
        )
        while True:
            try:
                ...
                # Concurrency mode: a runner holds several experiments, so the
                # single `current_experiment_id` probe below would wrongly free
                # it. Reconcile by active count instead — free only when none
                # remain (per-experiment terminal detection happens elsewhere
                # via get_experiment_status).
                if concurrency_limit > 1:
                    for runner in busy_models:
                        active = await self.db_service.count_active_experiments_for_runner(runner.id)
                        if active <= 0:
