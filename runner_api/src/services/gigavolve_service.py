...
class GigaEvolveService:
    ...
    async def fetch_programs(
        ...
        """Load program payloads from ``<uuid>:program:*`` Redis keys."""
        ...
        try:
            ...
            logger.debug(f"fetch_programs failed for {experiment_id}: {exc}")
            return []
        finally:
            await client.aclose()
        ...
        for raw in raws:
            parsed = parse_program_payload(raw)
            if parsed is not None:
                programs.append(parsed)
        return programs

    async def completed_ga_generations(self, experiment_id: str) -> int:
        """Return completed GA rounds (display generation + 1) from Redis lineages."""
        programs = await self.fetch_programs(experiment_id)
        return completed_generations_count(programs)
    ...
    async def run_experiment(
        ...
        try:
            ...
            # Hydra append syntax: max_generations lives in constants/evolution.yaml
            # as null; plain override fails with "Could not override 'max_generations'".
            ...
            # cancellation or GA generation limit.  This avoids the deadlock
            # that occurs when the OS pipe buffer fills up and the child
            # blocks on write.
            comm_task = asyncio.ensure_future(proc.communicate())
            cancelled = False
            limit_reached = False
            max_iterations_int = int(max_iterations) if max_iterations else None
            ...
            while not comm_task.done():
                ...
                if max_iterations_int and max_iterations_int > 0:
                    try:
                        programs = await self.fetch_programs(experiment_id)
                        if ga_limit_reached(programs, max_iterations_int):
                            logger.info(
                                "GA generation limit %s reached for %s, stopping runner",
                                max_iterations_int,
                                experiment_id,
                            )
                            limit_reached = True
                        ...
                        logger.debug(
                            f"Generation limit check failed for {experiment_id}: {exc}"
                        )
