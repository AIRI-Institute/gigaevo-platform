...
class TaskWorker:
    ...
    async def _check_iterations_completed(
        ...
        try:
            ...
            if not report_path.exists():
                ...
                try:
                    actual_iterations = await self.gigavolve_service.completed_ga_generations(
                        experiment_id
                    )
                    if actual_iterations > 0:
                        is_complete = actual_iterations >= expected_max_iterations
                        logger.info(
                            f"Iterations from Redis lineages for {experiment_id}: "
                            f"{actual_iterations}/{expected_max_iterations}, complete={is_complete}"
                        )
                        return is_complete, actual_iterations
                except Exception as redis_exc:
                    logger.debug(
                        f"Redis lineage iteration check failed for {experiment_id}: {redis_exc}"
                    )
                return False, None
    ...
    async def _handle_run_experiment(self, task: Task):
        ...
        if result and result.get("timed_out", False):
            ...
            if result.get("generation_limit_reached"):
                actual_iterations = await self.gigavolve_service.completed_ga_generations(
                    experiment_id
                )
                task.status = TaskStatus.COMPLETED
                task.result = result
                logger.info(
                    f"Experiment {experiment_id} stopped at GA generation limit: "
                    f"{actual_iterations}/{max_iterations} iterations"
                )
            else:
                is_complete, actual_iterations = await self._check_iterations_completed(
                    experiment_id, max_iterations
                )
